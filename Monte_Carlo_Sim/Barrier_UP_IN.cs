...
{
 public class Barrier_UP_IN:option
    {
        public Barrier_UP_IN(double So, double k, double r, double T, int steps, double sigma, int trials, bool y, bool x, bool multithread, double barrier, double rebate) : base( So, k,  T, r, sigma, y, trials, steps, x,multithread,barrier,rebate)
        ...
        public double Get_price(double So, double k, double T, double r, double sigma, int trials, int steps, bool call, bool x, bool y, bool multithread, double[,] randn, out double SE,double barrier)
        {
            ...
            if (y)
            {
                ...
                for (int i = 0; i < St.GetLength(0); i++)
                {
                    if (call)
                    {
                        if (max[i, 0] > barrier)// payoff only if maximum stock price above the barrier
                        {
                    ...
                    {
                        if (max[i, 0] > barrier)// payoff if maximum stock price above the barrier,
                        { 
            ...
            else // without control variate
            {
                for (int i = 0; i< St.GetLength(0); i++)
                {
                    if (call)
                    {
                        if (max[i, 0] > barrier)//payoff only if maximum stock price stays above the barrier 
                        {
