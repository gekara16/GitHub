...
{
 public class Barrier_DOWN_OUT:option
    {
        public Barrier_DOWN_OUT(double So, double k, double r, double T, int steps, double sigma, int trials, bool y, bool x, bool multithread, double barrier, double rebate) : base( So, k,  T, r, sigma, y, trials, steps, x,multithread,barrier,rebate)
        ...
        public double Get_price(double So, double k, double T, double r, double sigma, int trials, int steps, bool call, bool x, bool y, bool multithread, double[,] randn, out double SE,double barrier)
        {
            ...
            if (y)// if control variate technique is  chosen then
            {
                for (int i = 0; i < St.GetLength(0); i++)
                {
                ...
                for (int i = 0; i < St.GetLength(0); i++)
                {
                    if (call)
                    {
                        if (min[i, 0] > barrier)// payoff if minimum stock price above the barrier
                        {
                            pay_off[i, 0] = Math.Max((St[i, steps - 1] - k), 0.0) + (beta * cv[i, 0]);
                        }
                    ...
                    {
                        if (min[i, 0] > barrier)// pay off if minimum stock price above the barrier
                        {
            ...
            else // without control variate i.e normal simulation
            {
                for (int i = 0; i < St.GetLength(0); i++)
                {
                    if (call)
                    {
                        if (min[i, 0] > barrier) //payoff if minimum stock price is above the barrier
                        {
                    ...
                    {
                        if (min[i, 0] > barrier)// pay_off if only minimum stock price stays above the barrier
                        {
