...
namespace Monte_Carlo_Sim
{
    ...
    {
        //constructor for Asian option class that is inherited from constructor of option class.
        public Asian (double So, double k, double r, double T, int steps, double sigma, int trials, bool y, bool x, bool multithread,double barrier,double rebate) : base( So, k,  T, r, sigma, y, trials, steps, x,multithread,barrier,rebate)
            ...
            if (y)
            {
                ...
                for (int i = 0; i < St.GetLength(0); i++)
                {
                    ...
                    for (int j = 1; j < steps; j++)
                    {
                        ...
                        if (call)
                        {
                            pay_off[i, 0] = Math.Max(average - k, 0.0) + (beta * cv[i, 0]);
                        }

                        else if (!call)
                        {
                            pay_off[i, 0] = Math.Max(k - average, 0.0) + (beta * cv[i, 0]);
                        }
                    }
                }
            }
            ...
            else
            {
                for (int i = 0; i < St.GetLength(0); i++)
                {
                    ...
                    for (int j = 1; j <St.GetLength(1); j++)
                    {
                        ...
                        if (call)
                        {
                            pay_off[i, 0] = Math.Max((average- k), 0.0);  //pay-off for call-option
                        }
                        else if (!call)
                        {
                            pay_off[i, 0] = Math.Max((k -average), 0.0);   //pay-off for put-option
