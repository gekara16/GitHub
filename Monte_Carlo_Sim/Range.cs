...
namespace Monte_Carlo_Sim
{
 public class Range:option
    {
        //constructor for Range option class that is inherited from constructor of option class.
        public Range (double So, double k, double r, double T, int steps, double sigma, int trials, bool y, bool x, bool multithread,double barrier,double rebate) : base( So, k,  T, r, sigma, y, trials, steps, x,multithread,barrier,rebate)
        ...
        public double Get_price(double So, double k, double T, double r, double sigma, int trials, int steps, bool call, bool x, bool y, bool multithread, double[,] randn, out double SE)
        {
            ...
            if (y)
            {
                for (int i = 0; i < St.GetLength(0); i++)
                {
                    for (int j = 1; j < steps; j++)
                    {
                        ...
                        double put_delta = d1.Blackscholes_delta(So, k, T-t, r, sigma) - 1;
                        if (call)
                        {
                ...
                for (int i = 0; i < St.GetLength(0); i++)
                {
                 ...
                 pay_off[i, 0] = (max[i,0] - min[i,0]) + (beta * cv[i,0]);
                }
            }
            ...
            else
            {
                for (int i = 0; i < St.GetLength(0); i++)
                { 
                  pay_off[i, 0] = (max[i, 0] - min[i, 0]);//pay-off for call-option
               
                   
                }
            }

            for (int i = 0; i <St.GetLongLength(0); i++)
            {
                sumoption += pay_off[i, 0];
            }
            Price = (sumoption / Convert.ToDouble(St.GetLongLength(0))) * Math.Exp(-r * T);//option price
